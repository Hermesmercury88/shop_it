...
// ---------------------------
if (!empty($item_names)) {
    $stmt->bind_param("sssds", $order_no, $customer_name, $customer_address, $grand_total, $pdf_file_path);
    if (!$stmt) {
        die("Prepare failed: " . $conn->error);
    }
    ...
    for ($i = 0; $i < count($item_names); $i++) {
        ...
        $stmt->bind_param("isissd", $receipt_id, $name, $qty, $unit, $price, $total);
        $stmt->execute();
    }
    $stmt->close();
}
...
echo "บันทึกข้อมูลเรียบร้อยแล้ว <a href='index.html'>กลับหน้าหลัก</a> หรืออยาก <a href='receipts2.html'>กรอกต่อ</a>";
?>
