            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<td class='editable col-total'>" . number_format($row['total'],2) . "</td>";
                        ...
                        $pdf_link = $pdf_file ? "../uploads/deliveries/" . $pdf_file : "#";  // ../ ขึ้นไปหนึ่งระดับ
                        echo "<td>";
                        echo "<a class='action-btn btn-viewpdf' href='{$pdf_link}' target='_blank'>ดู PDF</a>";echo "<input type='file' class='upload-pdf' data-delivery-id='{$row['delivery_id']}'>";
echo "<button class='action-btn btn-uploadpdf' onclick='uploadPDF(this)'>อัพโหลด PDF</button>";
echo "</td>";
                    echo "<td>
...
function uploadPDF(btn){
    let row = btn.closest("tr");
    let deliveryId = row.dataset.deliveryId;
    let input = row.querySelector(".upload-pdf");
    if(input.files.length === 0) { alert("โปรดเลือกไฟล์ PDF"); return; }
    let file = input.files[0];
    let formData = new FormData();
    formData.append("pdf_file", file);
    formData.append("delivery_id", deliveryId);

    fetch("upload_all_deliveries1.php", {
        method:"POST",
        body: formData
    })
    .then(res=>res.json())
    .then(data=>{
        if(data.success){
            alert("อัพโหลด PDF สำเร็จ");
            location.reload();
        } else {
            alert("อัพโหลดไม่สำเร็จ: "+data.message);
        }
    });
}
</script>
