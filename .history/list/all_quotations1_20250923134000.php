    ...
    btn.addEventListener("click", function() {
        ...
        if (this.textContent === "แก้ไข") {
            tds.forEach((td, index) => {
                ...
                // ✅ ถ้าเป็นช่อง qty (5) หรือ price (6) ให้คำนวณ total (8)
                if (index === 5 || index === 6) {
                    td.querySelector("input").addEventListener("input", function() {
                        let qty = parseFloat(row.querySelectorAll("td input")[5].value) || 0;
                        let price = parseFloat(row.querySelectorAll("td input")[6].value) || 0;
                        let total = qty * price;
                        row.querySelectorAll("td")[8].innerHTML = `<input value="${total.toFixed(2)}">`;
                    });
                }
            });
