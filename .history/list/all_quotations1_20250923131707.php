...
@media print {
    ...
    .sidebar, .search-box, .btn-pdf, th:last-child { display: none !important; }
...
document.getElementById("searchInput").addEventListener("keyup", function() {
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (cell) {
            rows[i].style.display = cell.textContent.toLowerCase().includes(input) ? "" : "none";
        }
    }
});
...
document.querySelectorAll(".btn-edit").forEach(btn => {
    btn.addEventListener("click", function() {
        ...
        if (this.textContent === "แก้ไข") {
            tds.forEach(td => { td.dataset.oldValue = td.textContent; td.innerHTML = `<input value="${td.textContent}">`; });
            ...
            cancelBtn.addEventListener("click", () => {
                tds.forEach(td => td.textContent = td.dataset.oldValue);
                this.textContent = "แก้ไข";
                this.className = "action-btn btn-edit";
                cancelBtn.remove();
            });
            ...
            fetch("update_quotation_item.php", {
                method: "POST",
...
document.querySelectorAll(".btn-delete").forEach(btn => {
    btn.addEventListener("click", function() {
        ...
        fetch("delete_quotation_item.php", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
