...
document.querySelectorAll(".btn-edit").forEach(btn => {
    btn.addEventListener("click", function() {
        ...
        if (this.textContent === "แก้ไข") {
            // แปลงเป็น input และเก็บค่า raw ไว้
            tds.forEach(td => {
                let val = td.textContent.trim();
                td.innerHTML = `<input type="text" value="${val}" data-raw="${val}">`;
            });
            ...
            function updateTotal() {
                ...
                let total = qty * price;
                totalCell.innerHTML = `<input type="text" value="${total.toFixed(2)}" data-raw="${total.toFixed(2)}">`;
            }
            ...
            // เก็บค่า input ส่งไป PHP (ส่งค่า raw เพื่อเก็บ .00 ด้วย)
            let data = [];
            tds.forEach(td => {
                let input = td.querySelector("input");
                data.push(input.dataset.raw ?? input.value);
            });
